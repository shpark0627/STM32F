...
void HAL_UART_RxCpltCallback(UART_HandleTypeDef *huart){
	...
	if(huart->Instance==USART1){
		HAL_UART_Transmit(&huart2, uart1_rx_buffer, sizeof(uart1_rx_buffer), 10);
	}
}
//void HAL_UARTEx_RxEventCallback(UART_HandleTypeDef *huart, uint16_t Size) {
//    if (huart->Instance == USART1) {
//        // Size는 실제로 수신된 바이트 수입니다.
//        // 받은 만큼만 PC(huart2)로 전송합니다.
//        HAL_UART_Transmit_IT(&huart2, uart1_rx_buffer, Size);
//
//        // 중요: DMA 수신 다시 활성화
//        HAL_UARTEx_ReceiveToIdle_DMA(&huart1, uart1_rx_buffer, sizeof(uart1_rx_buffer));
//    }
